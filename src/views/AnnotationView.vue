<template>
  <div class="navbar bg-base-100">
    ...
    <div class="flex-none">
      <TypeFilter v-model="selectedFilters" @update:model-value="changeSelectedFilters" />
    </div>
  </div>
    ...
    <div :class="[`p-4 border flex flex-col`, { 'w-1/2': showMetadata, 'w-2/3': !showMetadata }]">
      <div class="flex flex-row gap-4 items-center">
        <label class="label cursor-pointer gap-2">
          ...
          />
          <span class="label-text"> Enkel Gewijzigde annotaties ({{ annotationStore.stats.modified }})</span>
        </label>
        <label class="label cursor-pointer gap-2">
          <input type="checkbox" class="toggle toggle-sm" :checked="showOnlyDuplicates" @click="showDuplicates" />
          <span class="label-text"> Enkel Duplicaten ({{ annotationStore.stats.duplicates }})</span>
        </label>
        <label class="label cursor-pointer gap-2">
    ...
    <div :class="[` border p-4`, { 'w-1/3': !showMetadata, 'w-1/2': showMetadata }]">
      ...
      <div class="flex flex-row gap-2">
        <div>
          ...
          {{ annotationStore.stats.totalProcessed }}/{{ annotationStore.stats.total }}
        </div>
