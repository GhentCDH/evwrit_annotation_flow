  ...
  <div>
    ...
    <div class="texts-container">
      <div class="text-column">
        ...
        <AnnotatedText
          :annotations="filteredDataAnnotations"
          :lines="textLines"
        />
      </div>
      <div class="text-column">
      ...
      <div class="text-column">
        ...
        <div class="action-buttons">
          <button class="primary-button" @click="confirmAllAnnotations">
            Bevestig Alles
          </button>
          <button class="primary-button" @click="confirmSelectedAnnotations">
            Bevestig Aangevinkte
          </button>
          <button class="primary-button" @click="toggleSelectAll">
            {{ allSelected ? "Deselecteer" : "Selecteer" }} Alle Annotaties
          </button>
          <button class="secondary-button" @click="clearAllAnnotations">
            Annuleer Alle wijzigingen
          </button>
        </div>

        <div
          class="annotated-line"
          v-for="annotation in filteredModifiedAnnotations"
          :key="annotation.id"
        >
          <div class="annotation-header">
            ...
            <div class="annotation-actions">
              <input
                type="button"
                value="✔️"
                @click="confirmAnnotation(annotation)"
              />
              <input
                type="button"
                value="✖️"
                @click="cancelAnnotation(annotation)"
              />
            </div>
          </div>
          <AnnotationViewer
            :annotation="annotation"
            :originalAnnotation="originalAnnotations.get(annotation.id)"
            :textLines="textLines"
          />
        </div>
      </div>
...
const filteredDataAnnotations = computed(() =>
  filterAnnotations(originalAnnotations.value, selectedFilters.value),
);
const filteredProcessedAnnotations = computed(() =>
  filterAnnotations(processedAnnotationsMap.value, selectedFilters.value),
);
const filteredModifiedAnnotations = computed(() =>
  filterAnnotations(modifiedAnnotationsMap.value, selectedFilters.value),
);
...
const applyRules = (nomalizedAnnotations: Map<string, RuleAnnotation>) => {
  ...
  const languageRuleSet = new AnnotationRuleSet(
    [sanitizeRule, tokenizeRule],
    true,
    true,
  );
  const typographyRuleSet = new AnnotationRuleSet(
    [sanitizeRule, textRule],
    true,
    true,
  );
  const orthographyRuleSet = new AnnotationRuleSet(
    [sanitizeRule, tokenizeRule, textRule],
    true,
    true,
  );
  const lexisRuleSet = new AnnotationRuleSet(
    [sanitizeRule, tokenizeRule],
    true,
    true,
  );
  const morphoSyntacticalRuleSet = new AnnotationRuleSet(
    [sanitizeRule, tokenizeRule],
    true,
    false,
  );
  const handshiftRuleSet = new AnnotationRuleSet(
    [sanitizeRule, tokenizeRule],
    true,
    true,
  );
  ...
  nomalizedAnnotations.forEach((nolmalizedAnnotation: RuleAnnotation) => {
    ...
    const processedAnnotaion = resultAnnotation.rule_applied
      ? resultAnnotation.annotation
      : nolmalizedAnnotation;
    if (resultAnnotation.rule_applied) {
      console.log("true");
      processedAnnotaion.class = "annotation--rule-applied";
      modifiedAnnotationsMap.value.set(
        nolmalizedAnnotation.id,
        processedAnnotaion,
      );
    }
    processedAnnotationsMap.value.set(
      nolmalizedAnnotation.id,
      processedAnnotaion,
    );
  });
...
const filterAnnotations = (
  annotationsMap: Map<string, RuleAnnotation>,
  selectedFilters: string[],
) => {
  if (showModified.value) {
    const modifiedAnnotations = Array.from(annotationsMap.values()).filter(
      (annotation) => modifiedAnnotationsMap.value.has(annotation.id),
    );
    if (selectedFilters.length === 0) return modifiedAnnotations;
    return modifiedAnnotations.filter((annotation: RuleAnnotation) =>
      selectedFilters.includes(
        originalAnnotations.value.get(annotation.id)?.type || "",
      ),
    );
  } else {
    if (selectedFilters.length === 0)
      return Array.from(annotationsMap.values());
...
const confirmSelectedAnnotations = async () => {
  try {
    const annotationsToSubmit = selectedAnnotationIds.value.map((id) =>
      modifiedAnnotationsMap.value.get(id),
    );
...
const toggleSelectAll = () => {
  if (allSelected.value) selectedAnnotationIds.value = [];
    ...
    selectedAnnotationIds.value = filteredModifiedAnnotations.value.map(
...
const confirmAllAnnotations = async () => {
  try {
    console.log(
      "filteredModifiedAnnotations",
      filteredModifiedAnnotations.value,
    );
