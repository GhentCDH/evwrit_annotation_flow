...
export class WordSnapper implements Snapper {
  ...
  fixOffset(newStart: number, newEnd: number) {
    const newAnnotationStartCharIndex =
      this.mapStartCharIndexToToken[newStart] ?? newStart;
    const newAnnotationStopCharIndex =
      this.mapStopCharIndexToToken[newEnd] ?? newEnd;
