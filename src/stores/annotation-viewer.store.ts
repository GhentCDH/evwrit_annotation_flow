...
import { AnnotationStore, type ConfirmAnnotationType } from "./annotation.store.ts";
...
export const useAnnotationStoreViewer = defineStore("useAnnotationStoreViewer" + Date.now(), () => {
  ...
  const debugRUle = async (annotationId: string) => {
  ...
  const updateView = (text: string, annotations: ModifiedAnnotation[]) => {
    annotationViewer.updateText(text).setAnnotations(annotations).setFilters({});
  };
